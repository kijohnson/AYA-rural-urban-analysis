...
analytic_dt_v_m <- analytic_dt_v %>%
  dplyr::select(stage_binary, RACE, AGE, SEX, educ,GCD, RUCC3,DX_LASTCONTACT_DEATH_MONTHS, dead, cantype) 

###########Function estimating mediation effect of stage for GCD and RUCC3 exposure###################
med<- function(analytic_dt_v_m) {
  x=analytic_dt_v_m[,c(1:5)] #covariates and mediators:stage_binary, RACE, AGE, SEX, and educ
  pred1=analytic_dt_v_m[,6] #exposure GCD
  pred2=analytic_dt_v_m[,7] #exposure RUCC3
...
########################Same function for cervix cancer (female only) only#########################
med_female<- function(analytic_dt_v_m) {
  x=analytic_dt_v_m[,c(1:3,5)] #covariates and mediators:stage_binary, RACE, AGE,and educ
  pred1=analytic_dt_v_m[,6] #exposure GCD
  pred2=analytic_dt_v_m[,7] #exposure RUCC3
  y=Surv(analytic_dt_v_m$DX_LASTCONTACT_DEATH_MONTHS, analytic_dt_v_m$dead) #surival outcome
  #n account for the randomness brought in by the sampling process. 
...
te <- t %>%
  ...
  coord_flip()+
  facet_wrap(~cantype, ncol=1, strip.position = "top",labeller = labeller(cantype= label_wrap_gen(40)))+
...
ggsave("Figure5.pdf", width = 9, height = 11, units ="in", dpi = 600)
```
...
ggsave("SupplementaryFigure1.pdf", width = 9, height = 12, units ="in", dpi = 600)
```
